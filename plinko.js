...
function createPlinkoBoard() {
  ...
  console.log(
    `Board dimensions: ${boardWidth}x${boardHeight}, Peg spacing: ${pegSpacing}`
  );
}
  ...
  function updateBall() {
    if (position.y < boardHeight - 20) {
      ...
      document.querySelectorAll(".peg").forEach((peg) => {
        ...
        const dx =
          (pegRect.left + pegRect.right) / 2 -
          (ballRect.left + ballRect.right) / 2;
        const dy =
          (pegRect.top + pegRect.bottom) / 2 -
          (ballRect.top + ballRect.bottom) / 2;
        const distance = Math.sqrt(dx * dx + dy * dy);

        if (distance < pegRadius + 10) {
          // 10 is ball radius
      ...
      if (position.x < pegRadius || position.x > boardWidth - pegRadius) {
        ...
        position.x =
          position.x < pegRadius ? pegRadius : boardWidth - pegRadius;
      }
